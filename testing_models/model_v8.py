...
#   -Convolution - 5x1 and 1x5 kernel
#
#   SECTION 3:
#   -Downsampling convolution - 5x5 kernel, 16 output channels
#   -Convolution - 5x5 and 1x5 kernel
#
#   SECTION 4:
#   -Convolution - 5x5 and 1x5 kernel
#   -Convolution - 5x5 and 1x5 kernel
...
layer_name = "s2_conv2"
with tf.name_scope(layer_name):
  W = utils.weight_variable([k_size, 1, ch[1], ch[1]])
  b = utils.bias_variable([ch[1]])
...
layer_name = "s2_conv3"
with tf.name_scope(layer_name):
  W = utils.weight_variable([1, k_size, ch[1], ch[1]])
  b = utils.bias_variable([ch[1]])
  conv = utils.conv2d(s2_conv2, W, b, 1)

  tanh = tf.nn.tanh(conv)
  s2_conv3 = tf.nn.dropout(tanh, keep_prob)
...
with tf.name_scope(layer_name):
  ...
  conv = utils.conv2d(s2_conv3, W, b, 2)
...
layer_name = "s3_conv2"
with tf.name_scope(layer_name):
  W = utils.weight_variable([k_size, 1, ch[2], ch[2]])
  b = utils.bias_variable([ch[2]])
...
layer_name = "s3_conv3"
with tf.name_scope(layer_name):
  W = utils.weight_variable([1, k_size, ch[2], ch[2]])
  b = utils.bias_variable([ch[2]])
  conv = utils.conv2d(s3_conv2, W, b, 1)

  tanh = tf.nn.tanh(conv)
  s3_conv3 = tf.nn.dropout(tanh, keep_prob)
...
layer_name = "s4_conv1"
with tf.name_scope(layer_name):
  W = utils.weight_variable([k_size, 1, ch[3], ch[3]])
  b = utils.bias_variable([ch[3]])
  conv = utils.conv2d(s3_conv3, W, b, 1)
...
layer_name = "s4_conv2"
with tf.name_scope(layer_name):
  W = utils.weight_variable([1, k_size, ch[3], ch[3]])
  b = utils.bias_variable([ch[3]])
...
with tf.name_scope(layer_name):
  ...
  tanh = tf.nn.tanh(conv)
  s4_conv3 = tf.nn.dropout(tanh, keep_prob)
...
layer_name = "s5_conv3"
with tf.name_scope(layer_name):
  W = utils.weight_variable([1, k_size, ch[4], ch[4]])
  b = utils.bias_variable([ch[4]])
  conv = utils.conv2d(s5_conv2, W, b, 1)

  tanh = tf.nn.tanh(conv)
  s5_conv3 = tf.nn.dropout(tanh, keep_prob)
...
layer_name = "s6_conv2"
with tf.name_scope(layer_name):
  W = utils.weight_variable([k_size, 1, ch[5], ch[5]])
  b = utils.bias_variable([ch[5]])
...
layer_name = "s6_conv3"
with tf.name_scope(layer_name):
  W = utils.weight_variable([1, k_size, ch[5], ch[5]])
  b = utils.bias_variable([ch[5]])
  conv = utils.conv2d(s6_conv2, W, b, 1)

  tanh = tf.nn.tanh(conv)
  s6_conv3 = tf.nn.dropout(tanh, keep_prob)
...
with tf.name_scope(layer_name):
  ...
  conv = utils.conv2d(s6_conv3, W, b, 1)
